...
RSpec.describe 'docker_service_postgresql' do
  let(:service_name){'service_postgresql'}
  ...
  context 'system tests for docker-service{postgres}', :system do
    ...
    it 'can find the container{service_postgresql}, matching against expected versions & configs' do
      container_postgres = audit_container(service_name, false, 'alpine', '3.12.1')

      expect(container_postgres.cmd!(%w(postgres --version))).to eq("postgres (PostgreSQL) 13.1")
      ...
      expect(env_vars['PG_VERSION']).to eq('13.1')
      expect(env_vars['PG_SHA256']).to eq('12345c83b89aa29808568977f5200d6da00f88a035517f925293355432ffe61f')
    end
