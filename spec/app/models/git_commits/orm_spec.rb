...
RSpec.describe 'git_commits.rb' do
        ...
        context 'handles needed scenarios' do
          ...
          context 'cases: error' do
            it 'bad commit_subject' do
              expect{GitCommit.spawn('1337', data_git_author_date, data_git_hash, nil, @fake_release_older)}.to raise_error(ActiveRecord::RecordInvalid)
            end
            it 'bad commit_hash' do
              expect{GitCommit.spawn('fake_str_a', data_git_author_date, 'yikes', nil, @fake_release_older)}.to raise_error(ArgumentError)
            end
            it 'bad commit_author_date' do
              expect{GitCommit.spawn('fake_str_a', 'yikes', data_git_hash, nil, @fake_release_older)}.to raise_error(ArgumentError)
            end
          end
        end
  ...
  end # end: {ApplicationRecord{GitCommit}}
